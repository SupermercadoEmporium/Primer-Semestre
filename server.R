...
shinyServer(function(input, output) {
  ...
  output$text1 <- renderPrint({ 
    x<-input$select
    paste(x, round(Prob_prod[x], digits=4))
    
  })
  
  output$text2 <- renderPrint({ 
    y<-input$select2
    paste(y,round(Prob_prod[y], digits=4))
  ...
  output$text4<-renderPrint({
    x<-input$select
    y<-input$select2
    paste("Lift",round(a_matrix[x,y]/(a_matrix[x,x]*a_matrix[y,y]),digits=4))
  })
  ...
  output$junio<-renderPrint({
    y<-input$select
    paste(y,round(Prob_prod_Junio[y], digits=4))
  })
  ...
  output$liftjunio<-renderPrint({
    x<-input$select
    y<-input$select2
    paste("Lift",round(a_matrix_Junio[x,y]/(a_matrix_Junio[x,x]*a_matrix_Junio[y,y]), digits=4))
  })
